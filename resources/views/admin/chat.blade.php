...
@section('title', 'Chat Pelanggan #' . $order->order_number)
...
@section('header')
    <div class="hidden md:flex items-center gap-3 text-[12px] font-black text-emerald-500 uppercase tracking-widest mb-2">
        ...
        <a href="{{ url('/admin/manajemen-pesanan') }}" class="hover:text-white transition-colors">Manajemen Pesanan</a>
        <span class="text-white/20">/</span>
        <span class="text-white">Chat Pelanggan</span>
    </div>
@endsection
...
@section('content')
    <div class="mb-8 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            <a href="{{ url('/admin/manajemen-pesanan') }}" class="w-12 h-12 bg-white border border-emerald-100 rounded-2xl flex items-center justify-center text-emerald-950 hover:bg-emerald-50 hover:scale-105 transition-all shadow-md">
            ...
            <div>
                <h1 class="text-[28px] md:text-[36px] font-black text-emerald-950 tracking-tighter leading-none mb-1.5">Chat Pelanggan</h1>
                <p class="text-emerald-700/60 font-bold text-[14px]">Percakapan langsung dengan pelanggan mengenai detail pesanan</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start mb-12">
        ...
        <div class="lg:col-span-2 bg-white rounded-[40px] border border-emerald-100 shadow-xl overflow-hidden flex flex-col h-[600px]">
            ...
            <div class="px-8 py-5 border-b border-emerald-50 bg-emerald-50/20 flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-[16px] font-black text-emerald-950 tracking-tight leading-tight">{{ $order->user->name ?? 'Pelanggan CIVAD' }}</h3>
                    <p class="text-[11px] text-emerald-500 font-bold uppercase tracking-wider flex items-center gap-1.5 mt-0.5">
                        <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span> Pelanggan
                    </p>
                </div>
            </div>
            ...
            <div class="p-6 border-t border-emerald-50 bg-white">
                <form id="chat-form" class="flex flex-col gap-3">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <input type="text" id="message-input" autocomplete="off" placeholder="Ketik balasan Anda ke pelanggan..." 
        ...
        <div class="bg-white rounded-[40px] border border-emerald-100 shadow-xl overflow-hidden p-8 space-y-6 lg:col-span-1">
            <div class="flex items-center gap-3 pb-4 border-b border-emerald-50">
                ...
                <h4 class="text-[14px] font-black text-emerald-950 uppercase tracking-[0.2em]">Ringkasan Transaksi</h4>
            </div>

            <!-- Order status and total -->
            <div class="space-y-4">
                <div>
                    <span class="text-[10px] font-black text-emerald-400 uppercase tracking-widest block mb-1">Status Pesanan</span>
                    @php
                        $statusColors = [
                ...
                <div>
                    <span class="text-[10px] font-black text-emerald-400 uppercase tracking-widest block mb-1">Total Bill</span>
                    <span class="text-[20px] font-black text-emerald-950 tracking-tighter">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span>
                </div>

                <div>
                    <span class="text-[10px] font-black text-emerald-400 uppercase tracking-widest block mb-1">Waktu Beli</span>
                    <p class="text-[13px] font-bold text-emerald-950 leading-relaxed">{{ $order->created_at->format('d M Y, H:i') }} WIB</p>
                </div>
            </div>

            <!-- Recipient details -->
            <div class="pt-6 border-t border-emerald-50 space-y-3">
                <span class="text-[10px] font-black text-emerald-400 uppercase tracking-widest block">Penerima & Kontak</span>
                <p class="text-[14px] font-black text-emerald-950 leading-tight">{{ $order->recipient_name ?? $order->user->name }}</p>
                <p class="text-[12px] font-bold text-emerald-600">{{ $order->phone_number ?? '-' }}</p>
            </div>
            ...
            <div class="pt-6 border-t border-emerald-50 space-y-2">
                <span class="text-[10px] font-black text-emerald-400 uppercase tracking-widest block">Alamat Tujuan</span>
